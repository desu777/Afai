...
class ViolationTracker:
    ...
    def add_violation(self, ip: str) -> bool:
        ...
        if len(self.violations[ip]) >= VIOLATION_THRESHOLD:
            ...
            debug_print(f"🚨 [Security] Auto-blacklisted IP {ip} after {len(self.violations[ip])} violations")
            return True
        
        debug_print(f"⚠️ [Security] Violation #{len(self.violations[ip])} for IP {ip}")
        return False
    ...
    def is_auto_blacklisted(self, ip: str) -> bool:
        ...
        if datetime.now() > expiry_time:
            ...
            debug_print(f"✅ [Security] Auto-blacklist expired for IP {ip}")
            return False
    ...
    def remove_auto_blacklist(self, ip: str) -> bool:
        ...
        if ip in self.auto_blacklisted:
            del self.auto_blacklisted[ip]
            debug_print(f"👤 [Security] Manually removed auto-blacklist for IP {ip}")
            return True
        return False
    ...
    def cleanup_expired(self):
        ...
        for ip in expired_ips:
            del self.auto_blacklisted[ip]
            debug_print(f"🧹 [Security] Cleaned expired blacklist for IP {ip}")
