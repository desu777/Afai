...
class SessionManager:
    ...
    def _start_cleanup_task(self):
        ...
        try:
            ...
            debug_print("⚠️ [SessionManager] No event loop - cleanup will be manual")
        ...
        while True:
            try:
                ...
                for session_id in expired_sessions:
                    ...
                    debug_print(f"🗑️ [SessionManager] Expired session: {session_id}")
                
                if expired_sessions:
                    debug_print(f"🧹 [SessionManager] Cleaned up {len(expired_sessions)} expired sessions")
                
                # Run cleanup every minute
                await asyncio.sleep(60)
                
            except Exception as e:
                debug_print(f"❌ [SessionManager] Cleanup error: {e}")
                await asyncio.sleep(60)
    
    def generate_session_id(self) -> str:
        """Generate new session ID"""
        session_id = str(uuid.uuid4())
        debug_print(f"🆕 [SessionManager] Generated new session: {session_id}")
        return session_id
    ...
    def get_session_cache(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if current_time - session_data.get("last_seen", 0) > self.ttl_seconds:
            del self.session_cache[session_id]
            debug_print(f"⏰ [SessionManager] Session expired: {session_id}")
            return None
        
        # Update last seen
        session_data["last_seen"] = current_time
        debug_print(f"✅ [SessionManager] Retrieved cache for session: {session_id}")
    ...
    def update_session_cache(self, session_id: str, extended_cache: Dict[str, Any]):
        ...
        debug_print(f"💾 [SessionManager] Updated cache for session: {session_id}")
    ...
    def clear_session(self, session_id: str) -> bool:
        ...
        if session_id in self.session_cache:
            del self.session_cache[session_id]
            debug_print(f"🗑️ [SessionManager] Cleared session: {session_id}")
            return True
        return False
    ...
    def clear_all_sessions(self):
        ...
        debug_print(f"🗑️ [SessionManager] Cleared all {count} sessions")
    ...
    def manual_cleanup(self):
        ...
        if expired_sessions:
            debug_print(f"🧹 [SessionManager] Manually cleaned up {len(expired_sessions)} expired sessions")
