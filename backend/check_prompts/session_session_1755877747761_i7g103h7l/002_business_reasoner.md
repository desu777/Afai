...
**LLM Response Time:** 15737.65ms  
**Session:** session_1755877747761_i7g103h7l  
**Timestamp:** 2025-08-22 17:49:24  
**User Query:** 你好，
...
- **Analyze Context:** Before any other action, you must analyze the `你好，

我的SPS缸出了点问题。我的轴孔珊瑚 (Acropora)，特别是蓝色的，正在褪色，变得非常苍白。我还注意到一颗蔷薇珊瑚 (Montipora) 从底部开始缓慢的组织坏死（STN）。

我的参数很稳定：KH 8.0，钙 (Ca) 440，镁 (Mg) 1350，而且NO3和PO4用Salifert测试几乎检测不到。我目前在使用Balling方法，但没有添加额外的微量元素。

我该如何恢复鲜艳的颜色并阻止STN？我在找能增强蓝色和粉色的产品。` and `No previous conversation context` to infer the user's aquarium system type.
...
- Meticulously scan the `你好，

我的SPS缸出了点问题。我的轴孔珊瑚 (Acropora)，特别是蓝色的，正在褪色，变得非常苍白。我还注意到一颗蔷薇珊瑚 (Montipora) 从底部开始缓慢的组织坏死（STN）。

我的参数很稳定：KH 8.0，钙 (Ca) 440，镁 (Mg) 1350，而且NO3和PO4用Salifert测试几乎检测不到。我目前在使用Balling方法，但没有添加额外的微量元素。

我该如何恢复鲜艳的颜色并阻止STN？我在找能增强蓝色和粉色的产品。` for any mentioned Aquaforest products (e.g., "component 123", "bio pro s").
